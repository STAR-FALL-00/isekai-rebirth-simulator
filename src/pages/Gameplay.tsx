  ...
  return (
    <AnimatePresence>
      ...
      >
        ...
        >
          <motion.div
            className="font-display text-3xl sm:text-5xl font-black mb-2"
            style={{
              color: cfg.color,
...
function TopBar({
  ...
  return (
    <motion.div
      ...
      className="h-[52px] sm:h-[56px] shrink-0 bg-bg-primary border-b border-border-subtle flex items-center px-3 sm:px-4 justify-between z-20"
    >
      {/* Back + World */}
      <div className="flex items-center gap-2 sm:gap-3">
        <button
          onClick={onBack}
          className="w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-bg-tertiary border border-border-subtle flex items-center justify-center text-text-secondary hover:border-border-glow hover:text-text-primary transition-all duration-150 hover:scale-105"
        >
        ...
        <div
          className="flex items-center gap-1.5 sm:gap-2 px-2 sm:px-3 py-1 rounded-2xl text-xs sm:text-sm font-bold font-body"
      ...
      >
        <div className="font-display text-base sm:text-xl font-bold text-text-primary">第 {age} 年</div>
        <div className="text-[10px] sm:text-xs text-text-secondary font-body whitespace-nowrap">{realmName} · {age}/{maxAge}</div>
      </motion.div>

      {/* HP + Speed */}
      <div className="flex items-center gap-4">
        <div className="hidden sm:flex flex-col items-end w-[140px]">
        ...
        {/* Mobile HP: mini version */}
        <div className="flex sm:hidden flex-col items-end w-[80px]">
          <div className="flex items-center gap-1 text-[10px] font-body text-text-secondary mb-0.5">
            <svg width="10" height="10" viewBox="0 0 24 24" fill={isLow ? '#C84B4B' : '#4BC88A'} stroke="none">
              <path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z" />
            </svg>
            <span className="font-mono text-text-primary">{health}/{maxHealth}</span>
          </div>
        ...
        <div className="flex items-center gap-1 sm:gap-2">
          {[
            { key: 'pause', icon: (<svg width="14" height="14" className="sm:w-4 sm:h-4" viewBox="0 0 24 24" fill="currentColor"><rect x="6" y="4" width="4" height="16" /><rect x="14" y="4" width="4" height="16" /></svg>), active: !autoPlay && !fastForward, onClick: () => { setAutoPlay(false); setFastForward(false); } },
            { key: 'play', icon: (<svg width="14" height="14" className="sm:w-4 sm:h-4" viewBox="0 0 24 24" fill="currentColor"><path d="M8 5v14l11-7z" /></svg>), active: autoPlay && !fastForward, onClick: () => { setAutoPlay(true); setFastForward(false); } },
            { key: 'ff', icon: (<svg width="14" height="14" className="sm:w-4 sm:h-4" viewBox="0 0 24 24" fill="currentColor"><path d="M4 18l8.5-6L4 6v12zm9-12v12l8.5-6L13 6z" /></svg>), active: fastForward, onClick: () => { setFastForward(true); setAutoPlay(true); } },
          ].map((btn) => (
            <button
              key={btn.key}
              onClick={btn.onClick}
              className="w-7 h-7 sm:w-8 sm:h-8 rounded-md flex items-center justify-center border transition-all duration-150"
              style={{
...
export default function Gameplay() {
  ...
  return (
    <div className="flex-1 flex flex-col bg-bg-primary overflow-hidden relative pb-[env(safe-area-inset-bottom)]">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        {/* Mobile Stats Toggle + Mini Panel */}
        <div className="md:hidden flex flex-col">
          <button
            onClick={() => setShowMobileStats((p) => !p)}
            className="flex items-center justify-between px-4 py-2 bg-bg-secondary border-b border-border-subtle text-xs font-body text-text-secondary"
          >
            <div className="flex items-center gap-2">
              <span className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: themeColor }} />
              <span>{identity.name}</span>
              <span className="text-text-muted">·</span>
              <span>{(WORLD_REALM_TABLES[world?.id ?? 'cultivation']?.[state.realm]?.name) ?? '未知境界'}</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="font-mono text-text-primary">{state.stats.health}/{100}</span>
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className={`transition-transform duration-200 ${showMobileStats ? 'rotate-180' : ''}`}>
                <path d="M6 9l6 6 6-6" />
              </svg>
            </div>
          </button>
          <AnimatePresence>
            {showMobileStats && (
              <motion.div
                ...
                transition={{ duration: 0.2 }}
                className="overflow-hidden bg-bg-secondary border-b border-border-subtle"
              >
        ...
        <div
          ref={eventLogRef}
          className="flex-1 overflow-y-auto bg-bg-primary/50 p-3 sm:p-6 space-y-3 relative"
          ...
          <AnimatePresence mode="wait">
            {currentEvent && (
              ...
              >
                ...
                <p className={`text-base sm:text-lg font-body leading-relaxed mb-4 ${isDying ? 'text-accent-red' : 'text-text-primary'}`} style={{ whiteSpace: 'pre-wrap' }}>
      ...
      <motion.div
        ...
        className="h-[44px] sm:h-[48px] shrink-0 bg-bg-primary border-t border-border-subtle flex items-center px-3 sm:px-4 justify-between z-20"
      >
        <div className="flex items-center gap-2">
          <button
      ...
      <motion.button
        ...
        className="fixed bottom-4 right-4 sm:bottom-6 sm:right-6 z-50 flex items-center gap-2 px-5 sm:px-6 py-2.5 sm:py-3 rounded-full font-body text-sm font-bold text-bg-primary disabled:opacity-40 disabled:cursor-not-allowed transition-all duration-150 shadow-lg"
        style={{
